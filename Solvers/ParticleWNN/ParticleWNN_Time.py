# /*
#  * @Author: yaohua.zang 
#  * @Date: 2024-09-30 17:09:45 
#  * @Last Modified by:   yaohua.zang 
#  * @Last Modified time: 2024-09-30 17:09:45 
#  */
import torch
import time
import os
...
class Solver(Module.Solver):
    ...
    def callBack(self, loss_eq, loss_bd, loss_all, errors, t_start):
        '''call back
        '''
        self.loss_eq_list.append(loss_eq.item())
        self.loss_bd_list.append(loss_bd.item())
        self.loss_list.append(loss_all.item())
    ...
    def train_setup(self, model_dict:dict, lr:float=1e-3, 
                    optimizer='Adam', scheduler_type='Plateau',
    ...
    def train(self, LossClass:Module.LossClass, 
              x_in_train, t_in_train, R_train, 
              batch_size_in:int, epochs:int=1, 
              w_pde=1., w_bd=1., 
        ...
        for epoch in trange(epochs):
            for inx in index_loader:
                ...
                loss_in = lossClass.Loss_pde(x_in_train[inx].to(self.device),
                                             t_in_train[inx].to(self.device), 
                                             R_train[inx].to(self.device))
            ...
            try:
                loss_in = lossClass.Loss_pde(kwrds['x_in_val'].to(self.device),
                                             kwrds['t_in_val'].to(self.device),
                                             kwrds['R_val'].to(self.device))
            except: 
                loss_in = lossClass.Loss_pde(x_in_train.to(self.device),
                                             t_in_train.to(self.device),
                                             R_train.to(self.device))
            loss_bd = lossClass.Loss_bd()
            #
            loss_val = w_pde*loss_in + w_bd*loss_bd
            if loss_val.item()<self.best_loss_val:
                self.best_loss_val = loss_val.item()
                self.saveModel(kwrds['save_path'], 'model_particlewnn_bestloss', self.model_dict)
            #######################  The scheduler
        ...
        ########################
        self.saveModel(kwrds['save_path'], name='model_particlewnn_final', 
                       model_dict=self.model_dict)
        self.saveLoss(kwrds['save_path'], name='loss_particlewnn')
